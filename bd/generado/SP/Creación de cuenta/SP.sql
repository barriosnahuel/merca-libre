...
AS
	SELECT * FROM GOODTIMES.USUARIO U, GOODTIMES.EMPRESA E 
	WHERE U.EMPRESA_ID = E.CUIT AND
...
CREATE PROCEDURE GOODTIMES.EliminarCliente
@ID bigint

AS
BEGIN TRANSACTION;
    DECLARE @CLIENTE_ID bigint;
	SELECT @CLIENTE_ID = U.CLIENTE_ID FROM GOODTIMES.USUARIO U WHERE ID = @ID;
	
	BEGIN TRY
	
	DELETE FROM GOODTIMES.USUARIO
	WHERE ID = @ID;
	
	DELETE FROM GOODTIMES.CLIENTE
	WHERE ID = @CLIENTE_ID;
