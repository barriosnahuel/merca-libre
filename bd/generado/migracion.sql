...
--		FORMA_PAGO
        ...
        BEGIN
            ...
            BEGIN
				...
				BEGIN                        
					SET @username = GOODTIMES. GET_UNIQUE_USERNAME(@Publ_Cli_Nombre)
					SET @dir = @Publ_Empresa_Dom_Calle + ' ' + CONVERT(varchar, @Publ_Empresa_Nro_Calle) + ' ' + CONVERT(varchar, @Publ_Empresa_Piso) + ' ' + CONVERT(varchar, @Publ_Empresa_Depto)
				...
				ELSE
				BEGIN
					SET @Publ_Owner = (SELECT ID FROM GOODTIMES.EMPRESA WHERE CUIT = @Publ_Empresa_Cuit);
				END	
            END
